...
- **契约导出/导入（ZIP）**：单角色命运树（母版 + 全部子版）与多角色舞台目录均可一键导出为标准 `.zip`；导入支持 `.zip` 自动解压还原（自动防重名续号），分享 / 备份 / 迁移不再需要手动翻目录
...
- **跨平台**：Windows / macOS / Linux / Android / iOS
  ...
  - Android：契约可在「应用内部存储 ↔ 应用外部存储」间切换，支持导入 `.meph` / `.zip` 文件
  - iOS：契约存储于应用沙盒文档目录（系统限制，卸载即清除），支持导入 `.meph` / `.zip` 文件

## 📖 快速开始

1. **配置 LLM**：进入「设置 → LLM 配置」，填入 API Key / Base URL / Model（兼容 OpenAI / DeepSeek / Ollama）
2. **选择契约**：首页选择内置契约（浮士德 / 埃德蒙·唐泰斯 / 贞德·达尔克 / 少年亚瑟 / 吉尔伽美什）或导入自己的 `.meph` 文件 / `.zip` 压缩包（命运树或舞台整体导入）
...
4. **Android 正式签名（升级可覆盖安装）**：
   ...
   >
   > **最便捷的方式：首页契约/舞台卡 ⋮ 菜单「导出（ZIP）」**——将命运树（母版 + 全部子版）或多角色舞台打包为 `.zip` 保存到下载目录 / 电脑 / 云盘，安装新版本后通过首页「导入」直接还原。
   >
   > 手动备份（适用于 Android 外部存储模式）：
   > 1. 通过手机文件管理器或 **USB 连接电脑（MTP 模式）**打开
   >    `/storage/emulated/0/Android/data/yuelinghuashu.mephisto/files/Mephisto/contracts`
   > 2. 将其中**所有 `.meph` 文件**复制到下载目录、电脑或云盘
   > 3. 安装新版本后，使用首页**「导入」**功能重新导入备份的 `.meph` 文件
   >
