...
void main() {
  ...
  test('loadStage 探测各角色是否已有 .child.meph 存档（hasSave）', () async {
    ...
    File('${stageDir.path}/浮士德.child.meph').writeAsStringSync(
      '【角色名】\n浮士德\n\n【历史】\n- fate: 命运指引\n',
    );
    ...
    expect(
      characters.firstWhere((c) => c.roleName == '浮士德').hasSave,
      isTrue,
    );
    expect(
      characters.firstWhere((c) => c.roleName == '梅菲斯特').hasSave,
      isFalse,
    );
  });
  ...
  test('角色发现排除 .child.meph 存档（不把存档当独立角色）', () async {
    ...
    // 各自自动存档（叙事中生成）
    File('${stageDir.path}/阿周那.child.meph').writeAsStringSync(
      '【角色名】\n阿周那\n\n【历史】\n- fate: 开战\n',
    );
    File('${stageDir.path}/迦尔纳.child.meph').writeAsStringSync(
      '【角色名】\n迦尔纳\n\n【历史】\n- fate: 应战\n',
    );
  ...
  test('deleteStageRoleCard 级联删除母版角色卡及其子版存档', () async {
    ...
    expect(
      await deleteStageRoleCard(stageDir.path, 'Arjuna.meph'),
      isTrue,
    );
  ...
  test('deleteStageRoleChild 仅删除子版存档，不影响母版角色卡', () async {
    ...
    expect(
      await deleteStageRoleChild(stageDir.path, '浮士德.meph'),
      isTrue,
    );
    // 母版保留
  ...
  test('renameStage 重命名舞台目录（含内部文件不变）', () async {
    ...
    expect(
      File('${tempDir.path}/新舞台/角色.meph').existsSync(),
      isTrue,
    );
    ...
    // 非法新名（含路径分隔符）→ false
    expect(
      await renameStage('${tempDir.path}/再来一次', 'a/b'),
      isFalse,
    );
  });
}
