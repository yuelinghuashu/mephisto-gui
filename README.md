...
- **辅助任务模型**（多模型路由）：可独立配置记忆提取 / 压缩使用的模型，默认关闭（共用主配置），API Key / Base URL 留空时继承主配置
- **消息操作**：长按 / 右键消息可「复制 · 重新生成」
- **输入历史**：桌面端 ↑ / ↓ 回溯最近 5 条命运指引，Shift+Enter 换行、Enter 发送
- **导出叙事**：存档菜单一键将完整对话导出为 Markdown 阅读版（`角色-命运纪事.md`）
- **双语界面**：简体中文 / English 一键切换，界面文案全量本地化
- **轻量 Markdown 渲染**：叙事文本支持 **粗体**、_斜体_、标题、引用、列表、`行内代码`（零外部依赖）
- **规则热重载**：保存 `.meph` → 仅规则即时生效，对话/状态/记忆/历史全保留
- **多角色舞台**：契约目录下的一层子目录 = 舞台，多位角色可同场互动、独立存档
- **跨平台**：Windows / macOS / Linux / Android / iOS，内置契约含浮士德、基督山伯爵、贞德、少年亚瑟、吉尔伽美什（另有波拿巴党卧底、帝国金殿两个示范子版）；另有俱卢之野、卡美洛之陨、三教论道三个多角色舞台
...
- **HTTP**（OpenAI 兼容 + Ollama 本地 SSE 流式调用）
- **archive**（ZIP 打包 / 还原）
- **MephParser**（自研契约解析器）
- **freezed**（数据模型代码生成）
